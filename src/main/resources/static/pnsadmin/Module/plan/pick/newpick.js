...
$(function(){
    ...
    var colNamesArr = [
        ...
        {"Caption": "领用仓库", "Name": "StoreName",'Editable': false},
        {"Caption": "领用仓库id", "Name": "StoreRd",'Editable': false, "Hidden": true},
        {"Caption": "领用数量", "Name": "Num", 'Editable': false,Width:80},
   ...
   // var isclick=true;
    var onClicks = function (nodeinfo, handle) {
        ...
        request({url:"/Pick/GetNPickInfo",data: objData},function(Body){
            ...
            $("#shztss").text(str1);
            var PurMaData = Body.Data.PickDlInfo;
    ...
    $(".searchTree").click(function () {
        ...
        currentPage = 0;
        currentPage = treeSearchs("/Pick/GetAllNPickInfo","PickRd","PickCode","PickCode",condition,currentPage,config);
        if(currentPage < 0){
            condition = "";
            currentPage = temp;
        }
    ...
    $("#prev").on("click",function(){
        if(currentPage > 0){
            currentPage --;
            currentPage = treeSearchs("/Pick/GetAllNPickInfo","PickRd","PickCode","PickCode",condition,currentPage,config,list);
        }
    });
    ...
    var loadTree=function(){
        ...
        request({url: '/Pick/GetAllNPickInfo', data: {"ExecType": "00","InitData": JSON.stringify(InitData1),"PageInfo":JSON.stringify(pageInfo)}}, function (Body) {
           $("#jstree_demo1").empty();
    ...
    var lywl = {
        ...
        "event":{
            "onseardata":function (o) {
                ...
                var objDate={
                    "StoreRd":storeid,
                    "MaVerDesc":excludeSpecial(o.condition)
                };
                var xldata = [];
    ...
    $(".cSave").click(function(){
        ...
        var PickCode=$("#PurChCode").val().trim();
        var PickerRd=$("#Buyers").getseldata().UserRd;
        if(PickerRd==""){
            toastr.warning("领用人不能为空");
            return false;
        }
        var PrePickDate=$("#ArrivalTime").val().trim();
        if(PrePickDate==""){
        ...
        if(PKMaInfo===undefined || PKMaInfo.length==0){
        ...
        if((treeID == null || treeID == "") && $("#ExecType").val() == "00"){
          ...
          //  return;
            for(var a in PKMaInfo){
                var sp=PKMaInfo[a].MaVerRd;
                PKMaInfo[a].MaVerRd=sp;
                delete PKMaInfo[a].PKDtlRd;
                delete PKMaInfo[a].MaCode;
                delete PKMaInfo[a].MaDes;
                delete PKMaInfo[a].MaName;
                delete PKMaInfo[a].StoreName;
                delete PKMaInfo[a].addid;
            }
            ...
            request({url: "/Pick/SaveNPickInfo", data: {"ExecType": "00","busData": JSON.stringify(PKMaInfodata)}}, function (Body) {
                $(".xd").attr("a",Body.Data.PickRd);
                $("#cancel").attr("a",Body.Data.PickRd);
                $("#stat").show();
                $("#status").text("待执行");
                $("#shzt").show();
                $("#shztss").text("未下达");
            ...
            for(var a in PKMaInfo){
                if(PKMaInfo[a].PKDtlRd=="undefined"||PKMaInfo[a].PKDtlRd==""||PKMaInfo[a].PKDtlRd==null){
                    PKMaInfo[a].PKDtlRd="";
                }
                var sp=PKMaInfo[a].MaVerRd;
               // var sp1=sp.split("_");
            ...
            request({url: "/Pick/SaveNPickInfo", data: {"ExecType": "02","busData": JSON.stringify(PKMaInfodata)}}, function (Body) {
                layer.closeAll("dialog");
    ...
    $("#delete").click(function(){
        ...
        if(PickRd==null||PickRd==""){
            ...
            request({url: "/Pick/SaveNPickInfo", data: {"ExecType": "01","busData": JSON.stringify(newData1)}}, function (Body) {
                    layer.closeAll("dialog");
                    toastr.success(Body.MsgDes);
                    loadTree();//局部刷新
                    $(".cSave").attr("a","");
            ...
            if (ssStaatus != null || "" != ssStaatus) {
             ...
             /*   alert(_MaName+"--"+MaDes+"--"+MaName+"--"+_MaDes);*/
                $maName.showData1({
                    ...
                    $("#lymater").find(".inpSearch").val($(this).find(".MaCode").text()+"-"+$(this).find(".MaName").text());
                    temporaryData={
    ...
    $("#import").on("click",function () {
        ...
        layer.open({
            ...
            shadeClose: true,
            area: ['60%', '50%'],
            content:getBasePath()+"/Pick/importnpickPG"
        });
    });
    //导出备货清单
    $("#export").on('click', function () {
        if(treeID != "" && treeID != null){
            if((str!="待执行" || str!="进行中")&& str1!="已下达"){
            ...
            layer.confirm('确认要导出该领料单信息？', {
                ...
                layer.closeAll("dialog");
                var url = getBasePath() + "/Pick/exportnPickExcel";
    ...
    $(".xd").click(function(){
        ...
        request({url: "/Pick/SaveNPickInfo", data: {"ExecType": "03","busData": JSON.stringify(newData1)}}, function (Body) {
            layer.closeAll("dialog");
            toastr.success(Body.MsgDes);
            loadTree();//局部刷新
            $("#shztss").text("已下达");
    ...
    $("#cancel").click(function(){
        ...
        request({url: "/Pick/SaveNPickInfo", data: {"ExecType": "04","busData": JSON.stringify(newData1)}}, function (Body) {
            layer.closeAll("dialog");
            toastr.success(Body.MsgDes);
            loadTree();//局部刷新
            $("#shztss").text("已取消");
    ...
    var params_BOM = {
        ...
        "event":{
            "onseardata":function (o) {
                ...
                if(storeid2 == undefined || storeid2 == ''){
            ...
            "onclick": function (res) {
                ...
                request({url: '/BOM/GetBVInfo', data: {
    ...
    var store2 = {
        ...
        "event":{
            ...
            "onclick":function (res) {
                storeid2=res.StoreRd;
                storename2=res.StoreName;
            }
        }
    };
    $("#Store2").zc_select(store2);
});