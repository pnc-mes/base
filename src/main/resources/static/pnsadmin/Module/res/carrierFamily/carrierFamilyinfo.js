...
$(function () {
    ...
    var rule = [{
        //工具家族名称
        "ctlid": "CarrierFamilyName", //自定义名字：标签id名字
        "param": "CarrierFamilyName" //规则中自定义的名字：对应报文中的id字段
    }, {
        //描述
        "ctlid": "Description",
        "param": "Description"
    ...
    var onClicks = function (nodeinfo, handle) {
        ...
        var objBusData = JSON.stringify({"CarrierFamilyRd": treeID});
        var objData = {
            "ExecType": "00",
            "BusData": objBusData,
        };
        /*------------------获取点击之后一个节点的数据------------------*/
        request({url:"/CarrierFamily/GetCarrierFamilyInfo",data: objData},function(Body){
    ...
    $(".searchTree").click(function () {
        ...
        currentPage = 0;
        currentPage = treeSearchs("/CarrierFamily/GetAllCarrierFamilyInfo","CarrierFamilyName","CarrierFamilyName","CarrierFamilyName",condition,currentPage,config);
        if(currentPage < 0){
            condition = "";
            currentPage = temp;
        }
...
//保养计划
    ...
    var loaddata = function () {
        ...
        var treedataList = [];
        request({url:'/CarrierFamily/GetAllCarrierFamilyInfo', data: {"ExecType": "00","InitData":JSON.stringify(InitData),"PageInfo":JSON.stringify(pageInfo)}},function(Body){
            ...
            for (var i = 0; i < len; i++) {
                var tree = {
                    id: treeData[i].CarrierFamilyRd == undefined ? "" : treeData[i].CarrierFamilyRd,
                    name: treeData[i].CarrierFamilyName == undefined ? "" : treeData[i].CarrierFamilyName
                }
    ...
    $(".cDel").click(function () {
        if (treeID != undefined && treeID != null) {
            layer.confirm('确认要删除吗？', {
                    ...
                    request({url:"/CarrierFamily/SaveCarrierFamilyInfo", data: {"ExecType": "01", "busData": "{\"CarrierFamilyRd\":" + treeID + "}"}},function(Body){
    ...
    $("a[name='copy']").click(function () {
        if (treeID != undefined && treeID != null && treeID.trim() != "") {
            ...
            request({url:"/CarrierFamily/SaveCarrierFamilyInfo", data: {"ExecType": "03", "busData": "{\"CarrierFamilyRd\":" + treeID + "}"}},function(Body){
                toastr.success(Body.MsgDes);
                $("#ExecType").val("");
                treeID = null;
                condition = "";
    ...
    $(".cSave").click(function () {
        ...
        //新增设备信息
            if ((treeID == null || treeID == "") && $("#ExecType").val() == "00") {
                newData = {
                    "CarrierFamilyName": formData["CarrierFamilyName"],
                ...
                request({url:"/CarrierFamily/SaveCarrierFamilyInfo", data: {"ExecType": $("#ExecType").val(), "busData": JSON.stringify(newData)}},function(Body){
                    toastr.success(Body.MsgDes);
                    $("#ExecType").val("");
                    condition = "";
                    currentPage = 0;
            ...
            //编辑设备信息
                ...
                request({url:"/CarrierFamily/SaveCarrierFamilyInfo", data: {"ExecType": "02", "busData": JSON.stringify(newData)}},function(Body){
                    toastr.success(Body.MsgDes);
                    $("#ExecType").val("");
                    condition = "";
                    currentPage = 0;
