...
import pnc.mesadmin.service.CheckTypeService;
...
public class CheckTypeController {
    ...
    //获取页面
    @RequestMapping(value = "/CTPG",method = RequestMethod.GET)
    public String  specinfo() {
        return "qa/jyx/jylxinfo";
    }

    //获取检验类型列表信息
    @RequestMapping(value = "/GetAllCTInfo", method = RequestMethod.POST)
    @ResponseBody
    public BaseRes GetAllCTInfo(GetAllReq request) {
        ...
        try {
            baseResponse = service.GetAllCTInfo(InitDataFields, pageInfo);
    ...
    @ResponseBody
    public BaseRes GetCTInfo(GetAllReq getAllReq) {
        BaseRes baseResponse = new BaseRes();
        BaseResB baseResBody = new BaseResB();
        if ("00".equals(getAllReq.getExecType())) {
            GetCheckTypeInfoResponse request = CommonUtils.switchClass(GetCheckTypeInfoResponse.class, getAllReq.getBusData());
            if (getAllReq.getPageInfo() != null) {

            } else {
                try {
                    baseResponse = service.GetCTInfo(request);
                } catch (SystemException e) {
                    baseResBody.setMsgCode(e.getMsgCode());
                    baseResBody.setMsgDes(e.getMsgDes());
                    baseResponse.setBody(baseResBody);
    ...
    @ResponseBody
    public BaseRes SaveCTInfo(SaveReq saveReq) {
        BaseRes baseResponse = new BaseRes();
        BaseResB baseResBody = new BaseResB();
        GetCheckTypeInfoResponse request = CommonUtils.switchClass(GetCheckTypeInfoResponse.class, saveReq.getBusData());
        try {
            if ("00".equals(saveReq.getExecType())) { //新增
                baseResponse = service.AddCTInfo(request);
            } else if ("01".equals(saveReq.getExecType())) { //删除
                baseResponse = service.RmDelCTInfo(request);
                baseResBody.setMsgCode("0x00000");
                baseResBody.setMsgDes("删除成功");
                baseResponse.setBody(baseResBody);
            } else if ("02".equals(saveReq.getExecType())) { //修改
                baseResponse = service.AddSaveCTInfo(request);
                baseResBody.setMsgCode("0x00000");
